  ...
  return (
    <div className="max-w-6xl mx-auto px-5 sm:px-8 py-10">
      ...
      {events === null ? (
        ...
        <p className="text-sm text-[var(--color-muted-fg)] mt-8">No events yet — check back soon.</p>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mt-8">
          {events.map((e) => (
            ...
            >
              {e.coverImage && (
                <img src={e.coverImage} alt={e.title} className="w-full h-36 object-cover" />
              )}
              <div className="p-4">
                <h3 className="font-display font-semibold text-base leading-snug">{e.title}</h3>
                {e.excerpt && <p className="text-sm text-[var(--color-muted-fg)] mt-2 line-clamp-2">{e.excerpt}</p>}
                <p className="text-xs text-[var(--color-muted-fg)] mt-3">
                  {formatDate(e.eventDate)}{e.location ? ` · ${e.location}` : ''}
                </p>
              </div>
            </Link>
          ))}
